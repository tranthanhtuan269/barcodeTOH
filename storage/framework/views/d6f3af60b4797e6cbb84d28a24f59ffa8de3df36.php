						...
						<table id="barcode-grid"  cellpadding="0" cellspacing="0" class="toggle-vis" border="0" class="display" width="100%">
					        <thead>
					            <tr>
									...
									<?php if(count($setting_table)>0): ?>
										<?php for($k = 1; $k < count($setting_table)-1; $k++): ?>
						                	<th class="<?php echo e($setting_table[$k]->name); ?>-field"><?php echo e(ChangeText($setting_table[$k]->name)); ?></th>
										<?php endfor; ?>
									<?php else: ?>
...
<!-- 								  	<select class="form-control" id="sel1">
						...
						<script type="text/javascript">							
						    ...
						    $(document).ready(function() {
							    $('#create-barcode-btn').click(function(){
							      ...
							      if(undefined == typeof(numBarcode) || 0 >= numBarcode){
							          swal(settingSweetalerForPayment("<?php echo $messages_check_buy_barcode; ?>"))
							              .then((result) => {
							                  if (result.value) {
						        ...
						        dataTable = $('#barcode-grid')
					                ...
					                aaSorting: [],
					                stateSave: true,
					                ajax: "<?php echo e(url('/')); ?>/barcode/listBarCodeUserAjax",
					                columns: dataObject,
					                pageLength: 25,
					                colReorder: {
								        fixedColumnsRight: 1,
								    ...
								    fnServerParams: function ( aoData ) {
								      	...
								      	if(load_first>0){
											...
											$.ajax({
												type	: "POST",
												url		: "<?php echo e(url('/')); ?>/barcode/putStateBarcodeTable",
												data 	: data,
												beforeSend: function() {
						    ...
						    function removeItem($id){
								swal(settingSweetaler("<?php echo $messages_delete_barcode; ?>"))
							    .then((result) => {
							      if (result.value) {
							    	var data = {
									  	id:$id,
									...
									$.ajax({
										type: "POST",
										url: "<?php echo e(url('/')); ?>/barcode/del",
										data: data,
										beforeSend: function() {
										    $("#pre_ajax_loading").show();
										},
						    ...
						    $('#apply-all-btn').click(function (){
					    		...
					    		if ($id_list.length > 0) {
									swal(settingSweetaler("<?php echo e($messages_delete_barcode); ?>"))
								    .then((result) => {
								      if (result.value) {
									      	var $id_list = '';
									    	$.each($('.check-barcode'), function (key, value){
								    		...
								    		if($id_list.length > 0){
												...
												$.ajax({
													type: "POST",
													url: "<?php echo e(url('/')); ?>/barcode/delMulti",
													data: data,
													beforeSend: function() {
													    $("#pre_ajax_loading").show();
													},
						            ...
						            swal({
						              html: '<div class="alert-danger"><?php echo $messages_check_selected_delete; ?></div>',
						            })
					    		}
						    });
						</script>
